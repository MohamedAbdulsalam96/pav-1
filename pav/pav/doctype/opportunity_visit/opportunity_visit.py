...
class OpportunityVisit(TransactionBase):
	def get_feed(self):
		return _("To {0}").format(self.lead)
	...
	def validate_maintenance_date(self):
		if self.maintenance_type == "Scheduled" and self.maintenance_schedule_detail:
			item_ref = frappe.db.get_value('Opportunity Schedule Detail', self.maintenance_schedule_detail, 'item_reference')
			if item_ref:
				start_date, end_date = frappe.db.get_value('Opportunity Schedule Item', item_ref, ['start_date', 'end_date'])
	...
	def update_completion_status(self):
		if self.maintenance_schedule_detail:
			frappe.db.set_value('Opportunity Schedule Detail', self.maintenance_schedule_detail, 'completion_status', self.completion_status)

	def update_actual_date(self):
		if self.maintenance_schedule_detail:
			frappe.db.set_value('Opportunity Schedule Detail', self.maintenance_schedule_detail, 'actual_date', self.mntc_date)
			...
			for d in self.get('purposes'):
				if d.prevdoc_docname and d.prevdoc_doctype == 'Warranty Claim' :
					if flag==1:
						...
						nm = frappe.db.sql("select t1.name, t1.mntc_date, t2.service_person, t2.work_done from `tabOpportunity Visit` t1, `tabOpportunity Visit Purpose` t2 where t2.parent = t1.name and t1.completion_status = 'Partially Completed' and t2.prevdoc_docname = %s and t1.name!=%s and t1.docstatus = 1 order by t1.name desc limit 1", (d.prevdoc_docname, self.name))
	...
	def check_if_last_visit(self):
		...
		if check_for_docname:
			check = frappe.db.sql("select t1.name from `tabOpportunity Visit` t1, `tabOpportunity Visit Purpose` t2 where t2.parent = t1.name and t1.name!=%s and t2.prevdoc_docname=%s and t1.docstatus = 1 and (t1.mntc_date > %s or (t1.mntc_date = %s and t1.mntc_time > %s))", (self.name, check_for_docname, self.mntc_date, self.mntc_date, self.mntc_time))
