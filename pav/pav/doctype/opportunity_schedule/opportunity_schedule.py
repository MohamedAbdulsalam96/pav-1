...
class OpportunitySchedule(TransactionBase):
	@frappe.whitelist()
	def generate_schedule(self):
		self.set('schedules', [])
		frappe.db.sql("""delete from `tabOpportunity Schedule Detail`
...
@frappe.whitelist()
def make_opportunity_visit(source_name, target_doc=None, item_name=None, s_id=None):
	...
	def update_sales(source, target, parent):
		sales_person = frappe.db.get_value('Opportunity Schedule Detail', s_id, 'sales_person')
		target.service_person = sales_person
		target.serial_no = ''

	doclist = get_mapped_doc("Opportunity Schedule", source_name, {
		"Opportunity Schedule": {
			"doctype": "Opportunity Visit",
			"field_map": {
		...
		"Opportunity Schedule Item": {
			"doctype": "Opportunity Visit Purpose",
