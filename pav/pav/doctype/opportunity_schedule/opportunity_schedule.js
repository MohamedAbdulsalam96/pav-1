...
frappe.ui.form.on('Opportunity Schedule', {
	setup: function (frm) {
...
pav.pav.OpportunitySchedule = frappe.ui.form.Controller.extend({
	refresh: function () {
		...
		if (this.frm.doc.docstatus === 0) {
			...
			if (flag) {
				this.frm.add_custom_button(__('Opportunity Visit'), function () {
					let options = "";
					...
					me.frm.call('get_pending_data', {data_type: "items"}).then(r => {
						...
						let d = new frappe.ui.Dialog({
							...
							primary_action(values) {
								frappe.call({
									method: "pav.pav.doctype.opportunity_schedule.opportunity_schedule.make_opportunity_visit",
									args: {
...
$.extend(cur_frm.cscript, new pav.pav.OpportunitySchedule({ frm: cur_frm }));
