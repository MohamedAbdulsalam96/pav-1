...
def get_data(conditions, filters):
	data = frappe.db.sql("""
		SELECT
			...
			so.status, so.customer, so.base_net_total, so.sales_partner, so.from_date, so.to_date, soi.item_code,
			...
			(soi.base_net_amount - (soi.billed_amt * IFNULL(so.conversion_rate, 1))) as pending_amount,
